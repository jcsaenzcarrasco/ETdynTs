...
% \hline
 ...
 \hline
% \texttt{connectedRec} & \texttt{[Vertex]} $\to$ \texttt{Forest} $\to$ \texttt{[Bool]} & $O(m \log n)$ \\
% \hline   
% \texttt{linkRec} & \texttt{[Vertex]} $\to$ \texttt{Forest} $\to$ \texttt{Forest} & $O(m \log n)$ \\
% \hline 
% \texttt{cutRec} & \texttt{[Vertex]} $\to$ \texttt{Forest} $\to$ \texttt{Forest} & $O(m \log n)$ \\
% \hline
...
The first block (first three rows) contains the functions to perform dynamic tree operations. The following block (rows \mdel{6 and 7} \madd{4 and 5}) lists the functions that compute an unbounded, although finite, sequence of dynamic tree operations over the same forest $F$; the suffix \texttt{Rec} represents recursion. Finally, last two rows are the core functions, apart from the ones provided by Hinze and Paterson work \cite{FTs}, to perform dynamic tree operations. \tcb{PERHAPS explain the above table widely or paraphrasing ???}
